...
def OTA_Main(pdk: MappedPDK, width_route: float = 0) -> Component:
    """
    ...
    Core_HMovement = (
        (Bias_size[0] + Core_size[0]) / 2 + 2 * size_via23[0] + 4 * min_separation3
    )
    ...
    extra_move = 0
    unavailable_pos = [
        port_biasv1n_core - 0.25 - min_separation2 - width_route / 2,
        port_biasv1n_core + 0.25 + min_separation2 + width_route / 2,
    ]
    ...
    OTA_MAIN << straight_route(
        pdk, Bias.ports["VBIASP2_E"], aux_P2.ports["bottom_met_W"]
    )

    aux_N2 = OTA_MAIN << via_23
    aux_N2.movey(Bias.ports["VBIASN2_E"].center[1])
    aux_N2.movex(
        Bias.ports["VBIASN2_N"].center[0] + 2 * size_via23[0] + 3 * min_separation3
    )
    OTA_MAIN << straight_route(
        pdk, Bias.ports["VBIASN2_E"], aux_N2.ports["bottom_met_W"]
    )
